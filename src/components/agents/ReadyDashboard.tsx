...
export default function ReadyDashboard({ agent }: { agent: Agent }) {
  ...
  const type = TYPE_LABELS[agent.agent_type] ?? agent.agent_type.replace(/_/g, " ")
  ...
  return (
    <div className="flex flex-col gap-6 ag-fade">
      ...
      <div className="flex items-center justify-between">
        ...
        >
          <span
            ...
            className="ag-pulse"
          />
          <span style={{ fontFamily: "var(--font-syne)", fontSize: "12px", fontWeight: 600, color: "#22c55e" }}>
            {t("ready", "agentReady")}
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { icon: <FileText size={14} />, label: "FILES UPLOADED", value: String(filesCount) },
          { icon: <MessageSquare size={14} />, label: "MESSAGES SENT", value: String(messageCount) },
          { icon: <Zap size={14} />, label: "AGENT TYPE", value: type },
        ].map((stat) => (
          <div
            key={stat.label}
            style={{
